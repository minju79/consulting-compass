...
import { proofAssets, pageProofRules, proofStatusLabels } from "@/data/proofSystemRules";
    ...
    <AppLayout>
      ...
      <div className="rounded-lg border bg-accent/5 border-accent/20 p-4 mb-8">
        ...
        <ul className="text-xs text-muted-foreground space-y-1">
          ...
          <li>• 검증이 필요한 자산은 게시 전 반드시 사실 확인을 완료해야 합니다.</li>
        </ul>
      </div>
          ...
          {Object.entries(pageProofRules).map(([page, rules]) => (
            <div key={page} className="rounded-lg border bg-card p-5">
              <h4 className="font-semibold text-sm text-foreground mb-3 capitalize">{
                { homepage: "홈페이지", service: "서비스 페이지", industry: "산업/전문분야 페이지", caseStudy: "케이스 스터디", contact: "문의 페이지", about: "회사 소개" }[page] || page
              }</h4>
                ...
                <div>
                  ...
                  {rules.required.length > 0 ? (
                    <ul className="space-y-1">
                      {rules.required.map((id) => {
                        const asset = proofAssets.find((a) => a.id === id);
                        return <li key={id} className="text-xs text-foreground">{asset?.name || id}</li>;
                      })}
                    </ul>
      ...
      <SectionBlock id="asset-classification" title="자산 분류 체계">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="rounded-lg border bg-yellow-500/5 border-yellow-500/20 p-5">
            ...
            <InfoList items={["수치 출처 미확인", "로고 사용 허가 미확인", "'업계 최고' 등 검증 필요 표현", "파트너십 현재 유효성 미확인"]} />
          </div>
